...
![Apiforge — 多协议接口调试工作台](docs/screenshot.png)
...
- **非根路径部署**：前端为 SPA，若部署在子路径（如 `/apiforge/`），需在前端 `vite.config.ts`
  设置 `base: "/apiforge/"` 并重新 `npm run build`，同时后端 `SpaHandler` 的静态目录与回退路由需匹配该前缀。
- **生产环境强制 HTTPS**：建议在反向代理（Nginx / Caddy / LB）层终止 TLS，并将
  `config.yaml` 的 `proxy.require_https: true`（或环境变量 `APIFORGE_REQUIRE_HTTPS=true`）。
  开启后中继（`/ws/relay`）握手被强制要求走 TLS，避免 WebSocket/Socket.IO 的 token 经 query
  在非加密信道泄露（L1）。
- **管理员口令**：首次启动用 `APIFORGE_ADMIN_USERNAME` / `APIFORGE_ADMIN_PASSWORD` 覆盖默认
  管理员（默认用户名 `admin`）；未设置 `APIFORGE_ADMIN_PASSWORD` 时会生成随机强口令并在日志告警
